    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            */
            ...
            // 最近加入的MP检查之后，剔除不合格的点，优质点从检查集合中去掉
            MapPointCulling();
            ...
            /**
             * 创建新的MP
             * 1、公式关键帧集合，如果帧数不够，用时序上前面一些帧补上
             * 2、遍历邻近关键帧集合，每一帧与当前关键帧计算基础矩阵F12
             * 3、2d-2d，利用基础矩阵极线约束，bow加速匹配两帧特征点
             * 4、匹配点的筛选剔除，最后三角化，创建MP
            */
            CreateNewMapPoints();
            ...
            if(!CheckNewKeyFrames())
            {
                ...
                /**
                 * 融合MP
                 * 1、取当前帧的共视关键帧，共视关键帧的共视关键帧，如果是imu还要加入时序上相邻的若干帧，组成一个大的关键帧集合
                 * 2、当前帧MP点投影到关键帧集合中，更新MP
                 * 3、关键帧集合中的点投影到当前帧，更新MP
                 * 4、更新当前帧MP
                 * 5、更新边
                */
                SearchInNeighbors();
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                // 冗余关键帧剔除，90%的地图点可以被其他至少三帧关键帧观测到，认为是冗余关键帧
                KeyFrameCulling();
...
/**
 * 处理队列离所有的帧，调用ProcessNewKeyFrame，全部处理完
*/
...
void LocalMapping::MapPointCulling()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        // MP三帧之前已经有了，没有被删除，说明是个优质点，从检查集合中删除，下次不再检查
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        else
        {
            ...
            // 两帧距离/场景深度值，比值如果很小表示点离相机很远，匹配或者三角化误差都会很大，太远的点，像素误差一点点，空间坐标误差就特别特别大
...
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        // 如果关键帧冗余MP超过总MP的90%，认为该帧为冗余帧，待删除
