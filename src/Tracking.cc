...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    */
    ...
    {
        // 还未初始化或者离初始化时刻不到30帧；唯一区别是orb提取的特征点数量不同，初始化帧提取较普通帧5倍的数量
...
/**
 * IMU预积分
 * 有两种预积分器，分别是前一关键帧开始预积分，从前一帧开始预积分
*/
void Tracking::PreintegrateIMU()
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        {   
            ...
            // 非常精细！前一帧与imu不是完全对齐的，减去多余的这部分，时间从前一帧时刻算起
...
void Tracking::Track()
{
    ...
    // 第一帧之后从NO_IMAGES_YET变成NOT_INITIALIZED
    if(mState==NO_IMAGES_YET)
    {
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
         ...
         * 2、最佳匹配点条件：最佳匹配点要求距离小于阈值，并且要与次佳匹配点拉开距离
...
/**
 * Map初始化
 * 1、单目初始化的两帧，设为关键帧，加入map
 * 2、遍历三角化点，创建MP，添加观测，更新法矢深度等
 * 3、更新关键帧的边
 * 4、BA优化
 * 5、尺度初始化
*/
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        // 关键帧加入MP，记录对应特征点索引
        // [经验]特征点全在，另外记录特征点是否匹配成功、是否三角化成功，这样通过索引访问，逻辑也比较清晰
        pKFini->AddMapPoint(pMP,i);
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);

        // MP添加观测，记录在观测帧中的对应特征点
    ...
    // [经验]场景深度中值小于0，或者当前帧三角化点（MP）数量少于50，认为初始化不成功
