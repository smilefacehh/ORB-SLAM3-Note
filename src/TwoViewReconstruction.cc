...
/**
 * 功能
 * 2d-2d匹配，计算H、F，进而恢复R、t，三角化
 * 
 * 经验
 * 1、开辟两个线程计算H、F
 * 2、计算H、F之前，对特征点去中心化、缩放，计算之后再乘回来。有论文论证过它的精度会更高，数值稳定
 * 3、H、F的评判标准是双向重投影误差
 * 4、通过三角化成功点数来取候选R、t，标准是次佳与最佳有一定距离，同时视差要足够大，三角化点要足够多
*/
 ...
 * 2d-2d重建结构
 * 1、准备好8对点ransac的数据
 * 2、开辟两个线程，分别计算单应矩阵H、基础矩阵F
 * 3、从H、F中选择打分较高的来恢复R、t，三角化
 * [经验]单应适用于纯旋转、平面的情况，非平面用F
*/
...
/**
 * 2d-2d匹配点计算单应矩阵H，迭代ransac进行优化
 * x2 = H21 * x1
 * 1、特征点归一化，T*x=x_norm
 * 2、迭代ransac8对点计算单应H21，尺度恢复，H21=T2.inv()*H21*T1
 * 3、单应矩阵H计算2d-2d双向重投影误差，选择误差最小的H
 * @param vbMatchesInliers 对应mvMatches12的索引，如果为匹配inlier点，值为true
 * @param score            最佳H对应得分
 * @param H21              最佳单应H
*/
void TwoViewReconstruction::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    */
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 尺度恢复，H21=T2.inv()*H21*T1
        H21i = T2inv*Hn*T1;
        H12i = H21i.inv();

        // 单应矩阵H计算2d-2d双向重投影误差，vbCurrentInliers记录匹配点（剔除误差过大的匹配点）
        currentScore = CheckHomography(H21i, H12i, vbCurrentInliers, mSigma);

        // 记录误差最小的H，得分，匹配点（对应mvMatches12的索引）
...
/**
 * 2d-2d匹配点计算基础矩阵F，迭代ransac进行优化
 * x2 * F21 * x1 = 0
 * 1、特征点归一化，T*x=x_norm
 * 2、迭代ransac8对点计算基础矩阵F21，尺度恢复，F21=T2.inv()*F21*T1
 * 3、基础矩阵F计算2d-2d双向重投影误差，选择误差最小的F
 * x2*F21*x1 = 0
 * @param vbMatchesInliers 对应mvMatches12的索引，如果为匹配inlier点，值为true
 * @param score            最佳F对应得分
 * @param F21              最佳F
...
void TwoViewReconstruction::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    */
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 尺度恢复，F21=T2.inv()*F21*T1
        F21i = T2t*Fn*T1;

        // 基础矩阵双向投影误差，vbCurrentInliers记录匹配点（剔除误差过大的匹配点）
        currentScore = CheckFundamental(F21i, vbCurrentInliers, mSigma);

        // 记录误差最小的F，得分，匹配点（对应mvMatches12的索引）
        if(currentScore>score)
        {
            F21 = F21i.clone();
            vbMatchesInliers = vbCurrentInliers;
 ...
 * 计算单应矩阵
 * x2 = H21 * x1
 * 注：理论上4对匹配点，可以构造8个等式，就可以求解H了。H本身有9个参数，x需要以齐次坐标形式参与运算，那么x1、x2第三个元素都是1，导致H第三行的值可以任意缩放，
 * 相当于其中一个参数值可以任取，所以H的参数由9个变成8个。
 * 参考：《视觉SLAM十四讲》p147
 * [公式]单应矩阵构造，SVD求解
*/
 ...
 * 计算基础矩阵
 * x2 * F21 * x1 = 0
 * 注：F一共8个参数，至少需要8对点；另外F的秩为2，所以只有7个自由度
 * 参考：《视觉SLAM十四讲》p143
 * [公式]基础矩阵构造，SVD求解，最小奇异值置为0，以满足秩为2的性质
*/
...
/**
 * 单应矩阵H计算2d-2d双向重投影误差，vbMatchesInliers记录匹配点（剔除误差过大的匹配点）
 * [经验]根据单应公式，分别投影到x1、x2下计算误差和
*/
...
/**
 * 基础矩阵双向投影误差，vbMatchesInliers记录匹配点（剔除误差过大的匹配点）
 * [经验]F双向投影误差
*/
 ...
 * 从基础矩阵F恢复R、t，三角化
 * 1、本质矩阵 E = K.t() * F * K
 * 2、本质矩阵E分解，得到t、-t、R1、R2，组合起来一共四个解
 * 3、对候选R、t三角化所有点，剔除深度值为负、重投影误差超过阈值、视差平行的点，返回剩余正常点数量
 * 4、三角化成功的点太少，或者没有明显胜出的R、t，认为初始化失败，并且要求视差够大（取匹配点视差中最大的，为什么不是计算平均视差，存在outlier）
 * [经验]初始化成功与否判断
*/
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    */
    ...
    // 对候选R、t三角化所有点，剔除深度值为负、重投影误差超过阈值、视差平行的点，返回剩余正常点数量
    ...
    // 三角化成功的点太少，或者没有明显胜出的R、t，认为初始化失败
...
/**
 * 从单应矩阵H恢复R、t，三角化
 * 1、如何从H恢复R、t，参考论文公式 todo
 * 2、一共8个候选解，对候选R、t三角化所有点，剔除深度值为负、重投影误差超过阈值、视差平行的点，返回剩余正常点数量
 * 3、次佳与最佳有一定距离，同时视差要足够大，三角化点要足够多
 * [经验]初始化成功与否判断
*/
...
void TwoViewReconstruction::Triangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D)
{
     ...
     *    v = P1.row(1) * P  (2)
     *    1 = P1.row(2) * P  (3)
     * 式(3)分别乘上u、v与(1)(2)式相减得到
     *    0 = [u * P1.row(2) - P1.row(0)] * P
     *    0 = [v * P1.row(2) - P1.row(1)] * P
     * 对于kp2，同样的操作，就化成了A*P=0的形式了
    */
    cv::Mat A(4,4,CV_32F);
