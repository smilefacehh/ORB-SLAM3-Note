...
/**
 * 更新关键帧的边，与共同观测帧建立边
 * 1、关键帧与共同观测点较多的帧建立边，权重为共同观测点数量
 * 2、如果共同观测点都不多，则与最多的一帧建立边
*/
void KeyFrame::UpdateConnections(bool upParent)
{
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>=th)
        {
            // 如果共同观测点够多，就与之建立一条边，边的权重为共同观测数量
            // [经验]与共同观测点较多的帧建立边，边的权重为观测数量
            vPairs.push_back(make_pair(mit->second,mit->first));
    ...
    if(vPairs.empty())
    {
        // 如果所有的观测帧与当前帧共同观测点都不够多，那么选最多的那一帧，建立边
...
{   
...

    {
        ...
        while(!mspChildrens.empty())
        {
            ...
            KeyFrame* pC;
            KeyFrame* pP;
            // 遍历孩子节点，在孩子节点的共视关键帧集合中，找到一帧，它与某孩子节点的共视点数最多，同时这一帧也是该孩子节点的爷爷节点
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    for(int i=0; i<N; i++)
    {
        if(mvpMapPoints[i])
        {
            // 计算深度值，完整的矩阵运算是RP+t，因为只需要深度值，所以取对应的行、列运算即可
            MapPoint* pMP = mvpMapPoints[i];
